...
const runs = computed(() =>
  allRuns.value.filter(r => r.phase === 'Failed' || r.phase === 'Stopped')
)
        ...
        <table class="tpl-table">
          ...
          <tbody>
            <tr v-for="r in runs" :key="r.name" class="clickable-row" @click="router.push('/pipelines/runs/' + r.name)">
              ...
              <td class="col-message" :title="r.message ?? ''">
                <q-tooltip v-if="r.message">{{ r.message }}</q-tooltip>
                {{ r.message || '—' }}
              </td>
            ...
            <tr v-if="!loading && runs.length === 0">
              <td colspan="9" class="empty-row">No failed or stopped runs.</td>
            </tr>
          </tbody>
        </table>
      </div>
