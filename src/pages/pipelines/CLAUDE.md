...
- `ExternalAuthRefPicker.vue` (`src/components/`) — mode select (`serviceAccount`|`oidc`) + name select backed by `weaveApi.fetchExternalAuthOptions()` (`GET .../external-auth/options`, static per-cluster allowlist, no store/cache needed — fetched on mount); used for `WeaveChainSpec.externalAuthRef` and `WeaveTriggerSpec.externalAuthRefOverride`, alongside the existing `SecretNamePicker` for `authSecretRef`. Both auth mechanisms are independent and stackable.
- `resetWeaveTrigger()` clears `WeaveTriggerStatus.quarantined` (set when a trigger's activation-source goroutine panics) via the same one-shot-annotation PATCH pattern as `fireWeaveTrigger`/`restart-step` (`fusion-platform.io/reset`) — generic to any trigger type, not gated behind the Kafka/BatchCron dedicated-endpoint split.
...
- BatchCron `jobs` entries: YAML or JSON list of `{ job: { id, schedule, name?, topic?, maintainer?, startdate?, starttime?, metadata? } }` (`job` wrapper key required, `fusion-flux/internal/trigger/batchjobs.go`) — JSON works fine since it's a valid YAML subset, useful when the list is externally autogenerated. Fields map 1:1 to `JOB_*` env vars injected into the run (`JOB_METADATA` is JSON-encoded).
- Creating a BatchCron trigger provisions a backing ConfigMap — `fusion-weave-api`'s Role needs `create` on core `configmaps`. This can be silently missing in a cluster even though the chart source has it, if past redeploys used `kubectl set image` instead of `helm upgrade` (Helm-templated RBAC never gets reapplied that way) — check `kubectl get role fusion-weave-api -n <ns> -o yaml` before assuming a handler bug on a `POST /batchtriggers` 500.
...
- Current semver: `0.5.0` (was `latest` — do not revert to `latest`)
- Both deployments are separate `Deployment` objects even though they share one image — a new operator env var (e.g. `EXTERNAL_AUTH_*`) must be set on **both** `fusion-weave-operator` and `fusion-weave-api` independently if the api-server binary also reads it (e.g. to serve it back via a REST endpoint); `kubectl set env` on one does not propagate to the other.
