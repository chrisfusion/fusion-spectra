...
const result  = ref<weaveApi.WeaveJobTemplateList | null>(null)
...
// detail / edit dialog
const selectedTemplate   = ref<weaveApi.WeaveJobTemplate | null>(null)
...
function openTemplateDialog(t: weaveApi.WeaveJobTemplate) {
...
function applySecCtx(spec: weaveApi.WeaveJobTemplateSpec) {
...
async function performUpdate(newSpec: weaveApi.WeaveJobTemplateSpec) {
  const t = selectedTemplate.value!
  editSubmitting.value = true
  editError.value      = null
  try {
    const updated = await weaveApi.updateJobTemplate(t, newSpec)
...
function saveEdit() {
  let newSpec: weaveApi.WeaveJobTemplateSpec
  try {
    newSpec = JSON.parse(editSpecJson.value) as weaveApi.WeaveJobTemplateSpec
  } catch {
...
function formatSpecJson(t: weaveApi.WeaveJobTemplate): string {
...
async function loadTemplates() {
  ...
  error.value   = null
  try {
    result.value = await weaveApi.listJobTemplates()
  } catch (e) {
    error.value = e instanceof Error ? e.message : 'Failed to load job templates'
  } finally {
    loading.value = false
  }
}
...
function templateAge(t: weaveApi.WeaveJobTemplate): string {
  const ts = t.metadata.creationTimestamp
  if (!ts) return '—'
  return new Date(ts).toLocaleString()
}

function confirmDelete(t: weaveApi.WeaveJobTemplate) {
  $q.dialog({
    ...
    try {
      await weaveApi.deleteJobTemplate(t.metadata.name)
      await loadTemplates()
    } catch (e) {
...
<template>
  <div class="page-grid">
    ...
    >
      <template #actions>
        <button
          v-if="can('weave:jobtemplates:write')"
          class="fs-btn fs-btn--primary"
          @click="router.push('/pipelines/weave/jobtemplates/create')"
        >
        ...
        <table class="tpl-table">
          ...
          <tbody>
            <tr v-for="t in pagedItems" :key="t.metadata.name" class="clickable-row" @click="openTemplateDialog(t)">
              ...
              <td class="col-num">{{ t.spec.parallelism ?? 1 }}</td>
              <td class="col-num">{{ t.spec.completions ?? 1 }}</td>
              <td>
              ...
              <td class="col-actions" @click.stop>
                <button
                  v-if="can('weave:jobtemplates:delete')"
            ...
            <tr v-if="!loading && pagedItems.length === 0">
              <td colspan="7" class="empty-row">No job templates found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <q-card class="tpl-dialog" v-if="selectedTemplate">
      <q-card-section class="tpl-dialog__header">
        <div class="tpl-dialog__title">
          <q-icon name="mdi-briefcase-outline" size="16px" />
