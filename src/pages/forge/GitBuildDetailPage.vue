...
const build        = ref<forgeApi.GitBuild | null>(null)
...
async function loadBuild() {
  ...
  try {
    build.value = await forgeApi.getGitBuild(buildId)
  } catch (e) {
...
async function loadLogs() {
  ...
  try {
    logs.value = await forgeApi.getGitBuildLogs(buildId)
    scrollLogs()
  } catch (e) {
...
const METADATA_SOURCE_LABEL: Record<string, string> = {
  full:    'Full (name + version from pyproject.toml)',
  version: 'Version only (name provided)',
  ...
  <div class="page-grid">
    ...
    <div class="breadcrumb">
      ...
      <span class="breadcrumb__current">
        ...
        <template v-else>Git Build #{{ buildId }}</template>
      </span>
    </div>
    ...
    >
      <div v-if="build" class="meta-grid">
        ...
        <div class="meta-row">
          ...
          <span class="meta-value">{{ METADATA_SOURCE_LABEL[build.metadataSource] ?? build.metadataSource }}</span>
        </div>

        <div v-if="build.entrypointFile" class="meta-row">
          <span class="meta-label">Entrypoint</span>
          <span class="meta-value fs-mono">{{ build.entrypointFile }}</span>
        </div>
