...
const build        = ref<forgeApi.AppBuild | null>(null)
...
async function loadBuild() {
  ...
  try {
    build.value = await forgeApi.getAppBuild(buildId)
  } catch (e) {
...
async function loadLogs() {
  ...
  try {
    logs.value = await forgeApi.getAppBuildLogs(buildId)
    scrollLogs()
  } catch (e) {
  ...
  <div class="page-grid">
    ...
    <div class="breadcrumb">
      ...
      <span class="breadcrumb__current">
        ...
        <template v-else>App Build #{{ buildId }}</template>
      </span>
    </div>
    ...
    >
      <div v-if="build" class="meta-grid">
        ...
        <div v-if="build.runner" class="meta-row">
          <span class="meta-label">Runner</span>
          <span class="runner-badge">{{ build.runner }}</span>
        </div>
        ...
        <div class="meta-row meta-row--top">
          <span class="meta-label">Repo URL</span>
          <span class="meta-value fs-mono meta-value--wrap">{{ build.repoUrl }}</span>
        </div>

        <div class="meta-row">
          <span class="meta-label">Repo Ref</span>
          <span class="meta-value fs-mono">{{ build.repoRef }}</span>
        </div>

        <div v-if="build.projectDir" class="meta-row">
          <span class="meta-label">Project Dir</span>
          <span class="meta-value fs-mono">{{ build.projectDir }}</span>
        </div>

        <div v-if="build.baseDependenciesUrl" class="meta-row meta-row--top">
          <span class="meta-label">Base Deps</span>
...
.runner-badge {
  ...
  color: #e8732a;
  background: color-mix(in srgb, #e8732a 12%, transparent);
  border: 1px solid color-mix(in srgb, #e8732a 30%, transparent);
}
