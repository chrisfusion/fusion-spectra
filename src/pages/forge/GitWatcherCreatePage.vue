...
const stepLabels = ['Configuration', 'Review & Submit'] as const
...
const createdWatch = ref<forgeApi.GitWatcher | null>(null)

function buildPayload(): forgeApi.CreateGitWatcherPayload {
...
async function submit() {
  ...
  try {
    createdWatch.value = await forgeApi.createGitWatcher(buildPayload())
  } catch (e) {
...
<template>
  <div class="page-grid">
    ...
    <div class="breadcrumb">
      ...
      <span class="breadcrumb__current">Add Watcher</span>
    </div>

    <CanvasPanel title="Add GitOps Watcher" icon="mdi-source-branch-sync" :wide="true">

      <!-- ── Success state ── -->
      <div v-if="createdWatch" class="success-body">
        <q-icon name="mdi-check-circle-outline" size="48px" class="success-icon" />
        <p class="success-title">Watcher created</p>
        <p class="success-sub">
          <span class="fs-mono">{{ createdWatch.name }}</span>
          is now watching <span class="fs-mono">{{ createdWatch.spec.repoRef || 'main' }}</span>
          on <span class="fs-mono">{{ createdWatch.spec.repoURL }}</span>.
        </p>
        <div class="success-actions">
          <button class="fs-btn fs-btn--ghost" @click="router.push(`/forge/gitwatchers/${encodeURIComponent(createdWatch.name)}`)">
      ...
      <template v-else>
        ...
        <div v-if="step === 1" class="form-body">
          ...
          <div class="form-row form-row--top">
            ...
            <div class="field-wrap">
              ...
              <span class="field-hint">
                <template v-if="buildType === 'git'">Builds a Python venv from a git repository (pyproject.toml)</template>
          ...
          <div class="form-row">
            ...
            <div class="field-wrap">
              ...
              <span v-else class="field-hint">HTTP(S) git URL — pre-flight connectivity check on save</span>
            </div>
          </div>
          ...
          <template v-if="buildType === 'git'">
            ...
            <div v-if="metadataSource === 'manual'" class="form-row">
              ...
              <div class="field-wrap">
                ...
                <span v-else class="field-hint">Semver — ignored when watcher polls (watcher reads from pyproject)</span>
              </div>
            </div>
          ...
          <template v-if="useTokenSecret">
            <div class="form-row">
              ...
              <div class="field-wrap">
                <input
                  v-model="tokenSecretName"
                  class="fs-input fs-mono"
                  :class="{ 'fs-input--error': tokenSecretNameErr }"
                  placeholder="my-repo-token"
                />
            ...
            <div class="form-row">
              ...
              <div class="field-wrap">
                <input
                  v-model="tokenSecretKey"
                  class="fs-input fs-mono"
                  :class="{ 'fs-input--error': tokenSecretKeyErr }"
                  placeholder="token"
                />
        ...
        <div v-else class="form-body">
          ...
          <div class="review-table">
            ...
            <div class="review-row">
              <span class="review-key">Build Type</span>
              <span class="review-val">{{ buildType === 'git' ? 'Git Build' : 'App Build' }}</span>
            </div>
            <div class="review-row">
            ...
            <div class="review-row">
              <span class="review-key">Enabled</span>
              <span class="review-val">{{ enabled ? 'Yes' : 'No (disabled)' }}</span>
            </div>
          ...
          <div class="form-note">
            ...
            The backend will verify repository reachability on save. The watcher will start polling immediately if enabled.
          </div>
          ...
          <div class="form-actions">
            <button class="fs-btn fs-btn--ghost" :disabled="submitting" @click="step = 1">
            ...
            <button class="fs-btn fs-btn--primary" :disabled="submitting" @click="submit">
              ...
              {{ submitting ? 'Saving…' : 'Create Watcher' }}
            </button>
          </div>
...
/* Build type / metadata source toggle */
...
.toggle-track {
  ...
  background: var(--fs-border); transition: background var(--fs-ease); flex-shrink: 0;
}
...
/* Review table */
.review-table { border: 1px solid var(--fs-border); border-radius: 5px; overflow: hidden; }
