...
<!-- 2026-09-18 -->
### Added
- **Weave**: UI for fusion-flux's new `externalAuthRef`/`unsafeEnvironmentInjector` fields and trigger quarantine status. New `ExternalAuthRefPicker.vue` component (mode select + name picker backed by the new `GET /api/weave/api/v1/external-auth/options` allowlist endpoint) wired into the Advanced Chain Builder (chain-level `externalAuthRef` + `unsafeEnvironmentInjector` toggle) and Trigger Create (per-trigger overrides), both gated the same way as the existing `authSecretRefOverride` field (hidden for Kafka/BatchCron, which use dedicated flat-body endpoints). Chain detail page shows `externalAuthRef`/`unsafeEnvironmentInjector` alongside the existing `authSecretRef` row. Trigger list page shows a "Quarantined" badge (with reason/timestamp tooltip) and a Reset action (`weaveApi.resetWeaveTrigger`, PATCHes the `fusion-platform.io/reset` annotation) for triggers whose activation-source goroutine panicked. No BFF changes required — the new fields/endpoint already ride on existing `weave:chains:write`/`weave:triggers:write`/`weave:resources:read` route rules (fusion-bff 0.10.1).

<!-- 2026-09-13 -->
### Added
- **Wizards**: third shortcut wizard, **Git → BatchCron Job** (`GitBatchCronJobWizardPage.vue`, `/wizards/git-batchcron-job/create`) — same repo/branch/subfolder provisioning as the Batch Job wizard (`GitWatcher`/build/`"stable"` tag/`WeaveJobTemplate`/`WeaveChain`, reusing `useGitAppProvisioning.ts` unchanged), but wires up a `BatchCron` trigger instead of a single one. Since a BatchCron job list can run to thousands of autogenerated entries, the jobs config is pasted or uploaded as raw YAML/JSON text (no per-row form, no git-fetch yet) and validated via the existing `/batchtriggers/validate` endpoint before provisioning starts. No "fire now" step: unlike the other Git wizards, BatchCron entries each start on their own `schedule`/`startdate`/`starttime` via fusion-flux's internal scheduler — the generic fire annotation would create a bare run with no `JOB_*` env vars, so it's intentionally not used. Verified end-to-end on minikube: a real BatchCron trigger fired a run automatically on schedule with correct `JOB_*` env vars.
### Fixed
- **Wizards** (`useGitAppProvisioning.ts`, affects all three Git wizards): `waitForBuild` polled `GET /appbuilds` (list) in a loop, but fusion-forge's list endpoint reads straight from Postgres with no sync — only `GET /appbuilds/:id` lazily syncs the underlying CIBuild CR's status into the DB row. A build could finish successfully and the wizard would still hang on "Building artifact" forever. Now resolves the build's id from one list call, then polls it by id instead, which actually triggers the sync.
